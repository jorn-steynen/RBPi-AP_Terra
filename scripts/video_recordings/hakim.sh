...
# MinIO config
MINIO_ALIAS="bsaffer-minio"
MINIO_URL="https://minio-prod.bsaffer.iot-ap.be"
MINIO_ACCESS_KEY="video"
MINIO_SECRET_KEY="AjtSu7cmQaXB4Jv5KMwseq"
MINIO_BUCKET="bsaffer"
MINIO_PATH="videos/38e3cc8d-8cf0-4bc8-915f-83efbcb65301"

# One-time alias config (skip if already set)
mc alias set "$MINIO_ALIAS" "$MINIO_URL" "$MINIO_ACCESS_KEY" "$MINIO_SECRET_KEY" > /dev/null 2>&1
...
until mc cp "$VIDEO_FILE" "$MINIO_ALIAS/$MINIO_BUCKET/$REMOTE_FILE"; do
    ((RETRY_COUNT++))
    echo "[WARNING] Attempt $RETRY_COUNT of $MAX_RETRIES to upload to MinIO failed." >> "$LOG_FILE"
    if [ "$RETRY_COUNT" -ge "$MAX_RETRIES" ]; then
        echo "[WARNING] Upload to MinIO failed after $MAX_RETRIES attempts. Saving file for later transfer."
...
for FILE in "$VIDEO_DIR/pending_uploads/"*; do
    ...
    echo "[INFO] Attempting to upload pending file: $BASENAME"
    if mc cp "$FILE" "$MINIO_ALIAS/$MINIO_BUCKET/$MINIO_PATH/$BASENAME"; then
        echo "[INFO] Successfully uploaded pending file: $BASENAME"
        rm -f "$FILE"
    else
        echo "[ERROR] Failed to upload pending file: $BASENAME"
        echo "$(date +"%Y-%m-%d %H:%M:%S") - [ERROR] Failed to upload pending file: $BASENAME" >> "$LOG_FILE"
